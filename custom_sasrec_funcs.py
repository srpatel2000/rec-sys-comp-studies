...
def trainSASRec(sess, model, sampler, user_sequences, args):
...
def predictForUsers(model, sess, user_sequences, target_df, itemnum, maxlen, pred_batch_size=256):
...
def convertIDBackToRaw(predictions_df, user_int_id, item_int_id):
    ...
    predictions_df["top_10_asins"] = predictions_df["top_10_items"].apply(
        lambda items: [int_to_item.get(i) for i in items]
    )
...
def runSASRecPipeline(data_type="dense"):
    ...
    trainSASRec(sess, model, sampler, user_sequences, args)
    ...
    # save outputs
    outputs_dir = config.data_dir / "outputs/sas_rec"
